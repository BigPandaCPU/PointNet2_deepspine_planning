...
def createCirclePlaneActor(plane_center, plane_normal, color="cyan", radius=8.0, opacity=1.0):
    """
...
def fitPlaneActorFromPoints(pts, color="cyan", radius=10.0):
    """
...
def showActors(actors, window_name="show spine"):
    ren = vtkRenderer()
...
def createRotateMatrixAroundNormal(normal, rotate_angle):
    """
    func:绕着单位向量normal,旋转rotate_angle角度，返回旋转矩阵
    point_new = np.dot(point, R)
    """
    # 确保 r 是一个单位向量
    r = np.array(normal)
    ...
    # 计算旋转矩阵
    cos_alpha = np.cos(rotate_rad)
    sin_alpha = np.sin(rotate_rad)

    # 构造旋转矩阵
    R = np.array([[cos_alpha + R_x ** 2 * (1 - cos_alpha),
                   R_x * R_y * (1 - cos_alpha) - R_z * sin_alpha,
                   R_x * R_z * (1 - cos_alpha) + R_y * sin_alpha],

                  [R_y * R_x * (1 - cos_alpha) + R_z * sin_alpha,
                   cos_alpha + R_y ** 2 * (1 - cos_alpha),
                   R_y * R_z * (1 - cos_alpha) - R_x * sin_alpha],

                  [R_z * R_x * (1 - cos_alpha) - R_y * sin_alpha,
                   R_z * R_y * (1 - cos_alpha) + R_x * sin_alpha,
                   cos_alpha + R_z ** 2 * (1 - cos_alpha)]])
...
def getAreaOfClosedCurvePoints(points):
    """
    """
    area = 0.0
...
def getTheMinCutPlaneAreaAlongAxisY(center, normal, target_poly_data, max_step = 5.0):
    """
    func:对切面沿着某一条轴，在一定范围内移动，计算最小的那个截面。=
    center:截面初始的中心点
    normal:截面的法向量
    target_poly_data:脊柱的stl
    ...
    for cur_step in np.arange(-max_step, max_step, 0.5):
        cur_center = center + cur_step * normal
        cur_bound_points, cur_fit_center_point, cur_cut_plane_area = getClipedCenterPoints(
            cur_center, normal, target_poly_data)

        dis = np.sqrt(np.sum(np.square(cur_fit_center_point - cur_center)))
        # if dis > dis_threshold:
        #     continue
        # if cur_cut_plane_area < area_threshold:
        #     continue

        if len(cur_bound_points) < 10:
            continue

        cut_plane_areas.append(cur_cut_plane_area)
        bound_points.append(cur_bound_points)
        centers.append(cur_fit_center_point)

    min_index = np.argmin(np.array(cut_plane_areas))

    return cut_plane_areas[min_index], bound_points[min_index], centers[min_index]


def getTheMinCutPlaneArea(rotate_normal, target_normal, target_center, target_poly_data, max_rotate_angle=45, dis_threshold=5, area_threshold=20):
    """
    ...
    如果截面的中心点，与初始的中心点之间的距离超过20mm，就放弃该截面
    """
    cut_plane_areas = []
    rotate_matrixs = []
    bound_points = []
    centers = []
    for cur_rotate_angle in range(-max_rotate_angle, max_rotate_angle, 1):
        cur_rotate_matrix = createRotateMatrixAroundNormal(rotate_normal, cur_rotate_angle)


        target_normal_new = np.dot(target_normal, cur_rotate_matrix)
        cur_bound_points, cur_fit_center_point, cur_cut_plane_area = getClipedCenterPoints(
            target_center, target_normal_new, target_poly_data)

        dis = np.sqrt(np.sum(np.square(cur_fit_center_point - target_center)))
        if dis > dis_threshold:
            continue
        # if cur_cut_plane_area < area_threshold:
        #     continue
        ...
        rotate_matrixs.append(cur_rotate_matrix)
        cut_plane_areas.append(cur_cut_plane_area)
        bound_points.append(cur_bound_points)
        centers.append(cur_fit_center_point)


    if len(cut_plane_areas) == 0:
        return None, None, None, None
    else:
        min_index = np.argmin(np.array(cut_plane_areas))
        return cut_plane_areas[min_index], bound_points[min_index], rotate_matrixs[min_index], centers[min_index]


def getClipedCenterPoints(plane_center, plane_normal, poly_data):
    """
    func:计算切面与狭部的切面轮廓，并提取轮廓的中心点
    """
    ...
    # # 分离成独立的轮廓
    # connectivityFilter = vtk.vtkPolyDataConnectivityFilter()
    # connectivityFilter.SetInputData(cutPolyData)
    # connectivityFilter.SetExtractionModeToAllRegions()
    # connectivityFilter.ColorRegionsOn()
    # connectivityFilter.Update()
    # numberOfRegions = connectivityFilter.GetNumberOfExtractedRegions()
    # #print(f"Number of contours: {numberOfRegions}")
    #
    # centroids = []
    # areas = []
    ...
    #
    #     # 如果是封闭的，则进行三角化和面积计算
    #     dcel = vtk.vtkContourTriangulator()
    #     dcel.SetInputData(regionPolyData)
    #     dcel.Update()
    #     triangulatedPolyData = dcel.GetOutput()
    #
    #     # cur_actor = createActorFromPolydata(triangulatedPolyData)
    #     # showActors([cur_actor])
    #
    #     # 计算面积
    #     massProperties = vtk.vtkMassProperties()
    #     massProperties.SetInputData(triangulatedPolyData)
    #     area = massProperties.GetSurfaceArea()
    #     areas.append(area)
    #
    #     # 计算重心
    #     centerOfMassFilter = vtk.vtkCenterOfMass()
    #     centerOfMassFilter.SetInputData(triangulatedPolyData)
    #     centerOfMassFilter.SetUseScalarsAsWeights(False)
    #     centerOfMassFilter.Update()
    #     centroid = centerOfMassFilter.GetCenter()
    #     centroids.append(centroid)
    #     #print(f"Contour {regionId}: Area = {area}, Centroid = {centroid}")
    #
    ...
    points_vtk = points.GetData()
    points_np = np.asarray(points_vtk)
    ...
    while cells.GetNextCell(indices):
        ...
        for i in range(indices.GetNumberOfIds()):
            ...
            cur_points.append([point[0], point[1], point[2]])
        lineCount += 1
        if len(cur_points)<10:
            continue

        final_points.append(cur_points)
        #print("cur points center:", np.mean(np.array(cur_points), axis=0))
        centers.append(np.mean(np.array(cur_points), axis=0))
    for cur_points in final_points:
        if len(cur_points) < 10:
            print("cur num points:", len(cur_points))
            areas.append(99999)
        else:
            areas.append(getAreaOfClosedCurvePoints(cur_points))
    # print("\n")
    # for i in range(len(centers)):
    #     print(f"Line {i}: Area = {areas[i]}, Centroid = {centers[i]}")

    index = np.argmin(np.sum(np.square(np.array(centers) - plane_center), axis=1))
    return final_points[index], centers[index], areas[index]
