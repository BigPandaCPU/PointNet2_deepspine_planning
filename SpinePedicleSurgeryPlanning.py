...
def createAxisActors2(axis_center, axis_normalX, axis_normalY, axis_normalZ):
    """

    """
    all_actors = []

    axis_normalX_actor = createLineActor([axis_center, axis_center + 20.0 * axis_normalX],
                                               color='red')
    axis_normalY_actor = createLineActor([axis_center, axis_center + 20.0 * axis_normalY],
                                               color='green')
    axis_normalZ_actor = createLineActor([axis_center, axis_center + 20.0 * axis_normalZ],
                                               color='blue')
    all_actors.append(axis_normalX_actor)
    all_actors.append(axis_normalY_actor)
    all_actors.append(axis_normalZ_actor)

    return all_actors


def PedicleSurgeryPlanning(landmark_points_t, landmark_points_l, landmark_points_r, target_poly_data,
                           TwoCentersDistanceThreshold=3, window_name="show spine", save_axis_dir = "",
                           save_png_dir = ""):
    """
    ...
    top_points_center = np.mean(landmark_points_t, axis=0)
    top_points_center_actor = createSphereActor(top_points_center, radius=1.0, opacity=1.0, color='red')
    all_actors.append(top_points_center_actor)

    target_actor = createActorFromPolydata(target_poly_data, opacity=0.8)
    all_actors.append(target_actor)


    left_points_center = np.mean(landmark_points_l, axis=0)
    left_cut_plane_center = left_points_center

    right_points_center = np.mean(landmark_points_r, axis=0)
    right_cut_plane_center = right_points_center

    left_fit_plane_actor, left_fit_plane_center, left_fit_plane_normal = fitPlaneActorFromPoints(np.array(landmark_points_l), color='green')
    left_fit_plane_center_actor = createSphereActor(left_fit_plane_center, radius= 0.5, opacity=1.0, color='red')

    #all_actors.append(left_fit_plane_center_actor)

    #############  对拟合的左侧平面的法向进行判断    ###########
    left_reference_normal = normalizeVector(top_points_center - left_fit_plane_center)

    left_cut_plane_center = left_fit_plane_center
    if np.sum(np.dot(left_reference_normal, left_fit_plane_normal)) < 0:
        left_cut_plane_normal = -left_fit_plane_normal
    else:
        left_cut_plane_normal = left_fit_plane_normal
    left_angle = calculateAngle(left_reference_normal, left_cut_plane_normal)
    ...
    right_fit_plane_actor, right_fit_plane_center, right_fit_plane_normal = fitPlaneActorFromPoints(np.array(landmark_points_r), color='blue')
    right_fit_plane_center_actor = createSphereActor(right_fit_plane_center, radius= 0.5, opacity=1.0, color='red')
   #all_actors.append(right_fit_plane_center_actor)

    ############### 对拟合的右侧平面法向进行判断  ##########
    right_reference_normal = normalizeVector(top_points_center - right_fit_plane_center)
    right_cut_plane_center = right_fit_plane_center

    if np.dot(right_reference_normal, right_fit_plane_normal) < 0:
        right_cut_plane_normal = -right_fit_plane_normal
    else:
        right_cut_plane_normal = right_fit_plane_normal
    right_angle = calculateAngle(right_reference_normal, right_cut_plane_normal)
    if right_angle > 45:
        right_cut_plane_normal = right_reference_normal

    ###################################################
    # left_cut_plane_normal_actor = createLineActor([left_cut_plane_center, left_cut_plane_center+20*left_cut_plane_normal], color='green')
    # all_actors.append(left_cut_plane_normal_actor)
    #
    # right_cut_plane_normal_acotr = createLineActor([right_cut_plane_center, right_cut_plane_center+20*right_cut_plane_normal], color='blue')
    # all_actors.append(right_cut_plane_normal_acotr)

    top_fit_plane_actor, top_fit_plane_center, top_fit_plane_normal = fitPlaneActorFromPoints(np.array(landmark_points_t), color='red')
    #all_actors.append(plane_actor_t)

    top_fit_plane_normal_actor = createLineActor([top_fit_plane_center, top_fit_plane_center+20*top_fit_plane_normal], color='red')

    all_actors.append(left_fit_plane_actor)
    all_actors.append(right_fit_plane_actor)
    showActors(all_actors)



    left_bound_points, left_cut_center, left_cut_plane_area = getClipedCenterPoints(left_cut_plane_center, left_cut_plane_normal, target_poly_data)
    ...
    left_bound_points_actors = createPointsActor(left_bound_points, radius=0.2, opacity=1.0, color='green')
    all_actors.extend(left_bound_points_actors)

    right_bound_points, right_cut_center, right_cut_plane_area = getClipedCenterPoints(right_cut_plane_center, right_cut_plane_normal, target_poly_data)
    ...
    right_bound_points_actors = createPointsActor(right_bound_points, radius=0.2, opacity=1.0, color='blue')
    all_actors.extend(right_bound_points_actors)

    #showActors(all_actors)

    ############### 将切面的中心点，替换之前由4个特征点拟合的中心点   #############

    if np.sqrt(np.sum(np.square(left_cut_plane_center - left_cut_center))) < TwoCentersDistanceThreshold:
        left_cut_plane_center = left_cut_center
    if np.sqrt(np.sum(np.square(right_cut_plane_center - right_cut_center))) < TwoCentersDistanceThreshold:
    ...
    ########### 椎弓根坐标系展示 ##########
    # left_axis_actors = createAxisActors2(landmark_points_l_center, left_axis_normalX, left_axis_normalY, left_axis_normalZ)
    # all_actors.extend(left_axis_actors)
    #
    # right_axis_actors = createAxisActors2(landmark_points_r_center, right_axis_normalX, right_axis_normalY, right_axis_normalZ)
    # all_actors.extend(right_axis_actors)
    #
    # showActors(all_actors)
    ...
    left_cut_plane_area_min2, left_bound_points_min2, left_rotate_matrix_min2, left_center_min2 = \
        getTheMinCutPlaneArea(left_axis_normalZ_new, left_axis_normalY_new, left_points_center, target_poly_data,
                              dis_threshold=TwoCentersDistanceThreshold)
    ...
    right_cut_plane_area_min2, right_bound_points_min2, right_rotate_matrix_min2, right_center_min2 = \
        getTheMinCutPlaneArea(right_axis_normalZ_new, right_axis_normalY_new, right_points_center, target_poly_data,
                              dis_threshold=TwoCentersDistanceThreshold)
    ...
    top_plane_center = top_points_center
    ...
    left_cut_plane_center_actor = createSphereActor(left_cut_plane_center, radius=1.0, opacity=1.0, color='green')
    right_cut_plane_center_actor = createSphereActor(right_cut_plane_center, radius=1.0, opacity=1.0, color='blue')
    all_actors.append(left_cut_plane_center_actor)
    all_actors.append(right_cut_plane_center_actor)
    ...
    left_reference_line_actor  = createLineActor([left_cut_plane_center, top_points_center], color="green")
    right_reference_line_actor = createLineActor([right_cut_plane_center, top_plane_center], color='blue')
    # all_actors.append(left_reference_line_actor)
    # all_actors.append(right_reference_line_actor)
    ...
    pedicle_pipeline_R_normal = createPediclePipelineNormal(rot_angle, spine_axis_normal, project_point)
    ...
    pedicle_pipeline_L_normal = createPediclePipelineNormal(-rot_angle, spine_axis_normal, project_point)
    ...
    intersect_points_R = getIntersectPointsFromLineAndPolyData(right_cut_plane_center - 100.0 * pedicle_pipeline_R_normal,
                                                               ...
                                                               target_poly_data)
    if len(intersect_points_R) < 2:
        print("计算右侧椎弓根通道交点失败，交点不足两个")
    ...
    intersect_points_L = getIntersectPointsFromLineAndPolyData(left_cut_plane_center - 100.0 * pedicle_pipeline_L_normal,
                                                               ...
                                                               target_poly_data)
    if len(intersect_points_L) < 2:
        print("计算左侧椎弓根通道交点失败，交点不足两个")
    ...
    # save_points_dir = "E:/data/DeepSpineData/label26/landmark_points"
...
def  main():
    ...
    src_label_dir = "E:/data/DeepSpineData/label26/mask"
    save_axis_dir = "E:/data/DeepSpineData/label26/axis"
    save_png_dir = "E:/data/DeepSpineData/label26/plan_result_png"
    save_stl_dir = "E:/data/DeepSpineData/label26/stl"
    os.makedirs(save_axis_dir, exist_ok=True)
    ...
    for label_name in label_names:
        ...
        uniques = np.unique(itk_np)

        # if 20 not in uniques:
        #     continue
        for i, cur_label in enumerate(uniques):
            if cur_label < 8:
                continue
            if cur_label not in [24,]:
                continue

            cur_idx = np.where(itk_np == cur_label)
            if len(cur_idx[0]) < 5780.0/(spacing[0]*spacing[1]*spacing[2]):
                continue
                # idx2 = np.where(itk_np == (cur_label+1))
                # if len(idx1[0]) < len(idx2[0])*0.8:
                #     continue

            # try:
            stl_name = os.path.join(label_name.replace(".nii.gz", "_label_%02d.stl"%cur_label))

            cur_index = np.where(itk_np == cur_label)

            cur_img_np = np.zeros_like(itk_np)
            cur_img_np[cur_index] = 1

            spine_poly_data_nomral, _ = createPolyDataNormalsFromArray(cur_img_np, spacing, origin, get_largest_connect_region=True)


            save_stl_file = os.path.join(save_stl_dir, stl_name)
            saveSTLFile(save_stl_file, spine_poly_data_nomral)
            #spine_actor = createActorFromPolydata(, opacity=0.8, color='pink')
            spine_poly_data = createPolyDataFromSTL(save_stl_file)
            spine_actor = createActorFromSTL(save_stl_file)
            #showActors([spine_actor])

            #spine_points = getPointsFromPolyData(spine_poly_data, num_points=2 * POINT_NET_INPUT_POINTS)
            spine_points = getPointsFromSTL(save_stl_file, num_points=2 * POINT_NET_INPUT_POINTS)
            ...
            showActors(all_actors)
            PedicleSurgeryPlanning(top_points, left_points, right_points, spine_poly_data,
                                   window_name=stl_name, save_axis_dir=save_axis_dir,
                                   save_png_dir=save_png_dir)
            # except Exception as e:
            #     print("bad stl:", stl_name)

        #break


def createLandmarkPointsFromPickedPoints():
    stl_dir = "E:/data/DeepSpineData/label26/bad_landmark_stl"
    picked_points_dir = "E:/data/DeepSpineData/label26/bad_landmark_stl"

    landmark_names = os.listdir(picked_points_dir)
    for landmark_name in landmark_names:
        if "Test10" not in landmark_name:
            continue
        if "picked_points.pp" not in landmark_name:
            continue
        ...
        save_axis_dir = "E:/data/DeepSpineData/label26/axis"
        save_png_dir = "E:/data/DeepSpineData/label26/plan_result_png"
        PedicleSurgeryPlanning(top_points, left_points, right_points, target_poly_data, window_name=stl_name, save_png_dir=save_png_dir, save_axis_dir=save_axis_dir)

def PedicleSurgeryPlanningFromStl():
    """
    """
    STL_SAMPLE_POINTS = 5000
    POINT_NET_INPUT_POINTS = 5000
    ...
    save_axis_dir = "E:/data/DeepSpineData/label26/axis"
    save_png_dir = "E:/data/DeepSpineData/label26/plan_result_png_new"
    src_stl_dir = "E:/data/DeepSpineData/label26/bad_stl"
    os.makedirs(save_axis_dir, exist_ok=True)
    os.makedirs(save_png_dir, exist_ok=True)


    stl_names = os.listdir(src_stl_dir)
    for stl_name in stl_names:
        ...
        # liver_161, [15]
        # liver_90, [17，19]
        if "1.3.6.1.4.1.9328.50.4.0266_seg_label_22" not in stl_name:
            continue

        try:
            stl_file = os.path.join(src_stl_dir, stl_name)
            ...
            spine_actor = createActorFromPolydata(spine_poly_data)
            top_points_actor = createPointsActor(top_points, radius=0.5, opacity=1.0, color="red")
            left_points_actor = createPointsActor(left_points, radius=0.5, opacity=1.0, color='green')
            right_points_actor = createPointsActor(right_points, radius=0.5, opacity=1.0, color='blue')
            # spine_points_actors = createPointsActor(spine_points, radius=0.2, opacity=1.0, color='red')
